...
export default function CapabilitiesPage() {
  ...
  const categories = Array.from(new Set(filteredCapabilities.map(c => c.category)));
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-neutral-400">Loading capabilities...</div>;
  }
    ...
    <div className="space-y-6 max-w-6xl">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-neutral-400">
          Explore the atomic skills available to your agents. 
          Manage which roles have access to specific system resources and tools.
        </p>
      </div>
                    ...
                    <Card className="h-full bg-neutral-900 border-neutral-800 group-hover:border-neutral-700 transition-colors">
                      <CardHeader className="p-4 pb-2">
                        <div className="flex justify-between items-start">
                          <Badge variant="outline" className="mb-2 text-xs font-normal border-neutral-700">
                            {cap.id}
                          </Badge>
                          <Badge variant={
                            cap.riskLevel === "critical" ? "destructive" :
                            cap.riskLevel === "high" ? "destructive" :
                            cap.riskLevel === "medium" ? "secondary" : "outline"
                      ...
                      <CardContent className="p-4 pt-0">
                        ...
                        <div className="mt-4 flex items-center gap-2 text-xs text-neutral-500">
                          ...
                          <span>Used by {getRolesWithCapability(cap.id).length} roles</span>
                        </div>
                      </CardContent>
                    </Card>
                  </motion.div>
      ...
      {/* Mapping Side Panel */}
      <Sheet open={!!selectedCapability} onOpenChange={(open) => !open && setSelectedCapability(null)}>
        <SheetContent className="sm:max-w-md bg-neutral-900 border-neutral-800 text-neutral-50">
          <SheetHeader className="mb-6">
            ...
            <div className="flex gap-2 mt-2">
              ...
              <Badge variant={
                selectedCapability?.riskLevel === "critical" || selectedCapability?.riskLevel === "high" 
                  ? "destructive" 
                  : "secondary"
              }>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <h4 className="text-sm font-medium text-neutral-400 uppercase tracking-wider">Role Access Control</h4>
            ...
            <ScrollArea className="h-[500px] pr-4">
              <div className="space-y-3">
                {roles.map(role => {
                  const hasAccess = role.capabilities.includes(selectedCapability?.id || "");
                  return (
                    <div 
                      key={role.id} 
                    ...
                    >
                      ...
                      <Switch 
                        checked={hasAccess}
                        onCheckedChange={() => selectedCapability && toggleRoleCapability(role, selectedCapability.id)}
                      />
                    </div>
                  );
                })}
            ...
            {selectedCapability?.riskLevel === "critical" && (
              <div className="bg-red-950/20 border border-red-900/50 p-4 rounded-lg flex gap-3 items-start">
                ...
                <div className="space-y-1">
                  <h4 className="text-sm font-semibold text-red-500">High Risk Capability</h4>
                  <p className="text-xs text-red-200/60">
                    Granting this capability allows unrestricted system access. 
                    Ensure stringent policies are in place for any role with this access.
                  </p>
                </div>
              </div>
            )}
